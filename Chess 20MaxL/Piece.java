...
public abstract class Piece
{
  //the board this piece is on
    ...
    /**
     * Checks is dest is a valid location.
     * @param dest the location to test 
    ...
    public boolean isValidDestination(Location dest)
    {
        ...
        if (!getBoard().isValid(dest)) 
            return false;
        if (getBoard().get(dest)!=null && getBoard().get(dest).getColor()==color) 
            return false;
        return true;
    }

    /**
     * Abstract method to find all locations the piece can move to.
     * @return the array list containing all locations the piece can move
     */
    public abstract ArrayList<Location> destinations();

    /**
     * Adds all locations in the given direction (stops at the end of
     * the board or when another piece is encountered) to dests. If a
     * piece of the opposite color is encountered, it is also added.
     * @param dests the list of locations the piece can move to
     * @param direction the direction in which to sweep
     */
    public void sweep (ArrayList<Location> dests, int direction)
    {
        Location loc = location;
        boolean stop = false;
        while ((isValidDestination(loc.getAdjacentLocation(direction)) && !stop))
        {
            dests.add(loc.getAdjacentLocation(direction));
            if (board.get(loc.getAdjacentLocation(direction))!= null && board.get(loc.getAdjacentLocation(direction)).getColor()!= this.getColor())
            {
                stop = !stop;
            }
